...
// This is a modified version of uart.c from ESP-IDF v5.5.4
...
/* @brief When calling `fnc_uart_set_pin`, instead of GPIO number, `UART_PIN_NO_CHANGE`
 *        can be provided to keep the currently allocated pin.
 */
#define FNC_UART_PIN_NO_CHANGE      (-1)

#define FNC_UART_FIFO_LEN _Pragma ("GCC warning \"'UART_FIFO_LEN' macro is deprecated, please use 'UART_HW_FIFO_LEN' instead\"") SOC_UART_FIFO_LEN ///< Length of the HP UART HW FIFO
#if (SOC_UART_LP_NUM >= 1)
#define FNC_UART_HW_FIFO_LEN(uart_num) ((uart_num < SOC_UART_HP_NUM) ? SOC_UART_FIFO_LEN : SOC_LP_UART_FIFO_LEN) ///< Length of the UART HW FIFO
#else
#define FNC_UART_HW_FIFO_LEN(uart_num) SOC_UART_FIFO_LEN                                                         ///< Length of the UART HW FIFO
#endif
#define FNC_UART_BITRATE_MAX        SOC_UART_BITRATE_MAX    ///< Maximum configurable bitrate
...
/**
 * @brief UART configuration parameters for fnc_uart_param_config function
 */
typedef struct {
...
/**
 * @brief UART interrupt configuration parameters for fnc_uart_intr_config function
 */
typedef struct {
...
} fnc_uart_intr_config_t;

/**
 * @brief UART event types used in the ring buffer
 */
typedef enum {
    FNC_UART_DATA,              /*!< Triggered when the receiver either takes longer than rx_timeout_thresh
                                 to receive a byte, or when more data is received than what rxfifo_full_thresh
                                 specifies*/
    FNC_UART_BREAK,             /*!< Triggered when the receiver detects a NULL character*/
    FNC_UART_BUFFER_FULL,       /*!< Triggered when RX ring buffer is full*/
    FNC_UART_FIFO_OVF,          /*!< Triggered when the received data exceeds the capacity of the RX FIFO*/
    FNC_UART_FRAME_ERR,         /*!< Triggered when the receiver detects a data frame error*/
    FNC_UART_PARITY_ERR,        /*!< Triggered when a parity error is detected in the received data*/
    FNC_UART_DATA_BREAK,        /*!< Internal event triggered to signal a break afte data transmission*/
    FNC_UART_PATTERN_DET,       /*!< Triggered when a specified pattern  is detected in the incoming data*/
#if SOC_UART_SUPPORT_WAKEUP_INT
    FNC_UART_WAKEUP,            /*!< Triggered when a wakeup signal is detected*/
#endif
    FNC_UART_EVENT_MAX,         /*!< Maximum index for UART events*/
} fnc_uart_event_type_t;

/**
 * @brief Event structure used in UART event queue
 */
typedef struct {
    fnc_uart_event_type_t type; /*!< UART event type */
    size_t size;            /*!< UART data size for FNC_UART_DATA event*/
    bool timeout_flag;      /*!< UART data read timeout flag for FNC_UART_DATA event (no new data received during configured RX TOUT)*/
 ...
 *
 * @note  Rx_buffer_size should be greater than FNC_UART_HW_FIFO_LEN(uart_num). Tx_buffer_size should be either zero or greater than FNC_UART_HW_FIFO_LEN(uart_num).
 *
 ...
 * @param rx_thresh Threshold of Hardware RX flow control (0 ~ FNC_UART_HW_FIFO_LEN(uart_num)).
 *        Only when FNC_UART_HW_FLOWCTRL_RTS is set, will the rx_thresh value be set.
 *
 * @return
 *     - ESP_OK   Success
 *     - ESP_FAIL Parameter error
 ...
 * @param thresh  Threshold of TX FIFO empty interrupt, 0 ~ FNC_UART_HW_FIFO_LEN(uart_num). If -1 is given, threshold configuration will be skipped.
 *
 * @return
 *     - ESP_OK   Success
 *     - ESP_FAIL Parameter error
 ...
 *       `fnc_uart_set_pin(0, 1, 3, -1, -1)` is performed, as GPIO1 is UART0's
...
/**
 * @brief Alias of fnc_uart_flush_input.
 ...
 *        In order to send all the data in tx FIFO, we can use fnc_uart_wait_tx_done function.
 * @param uart_num UART port number, the max port number is (UART_NUM_MAX -1).
 *
 * @return
 *     - ESP_OK Success
 ...
 * @note  In order to send all the data in tx FIFO, we can use fnc_uart_wait_tx_done function.
 * @param uart_num UART port number, the max port number is (UART_NUM_MAX -1).
 *
 * @return
 *     - ESP_OK Success
 ...
 *        fnc_uart_flush_input, fnc_uart_read_bytes, fnc_uart_driver_delete, uart_pop_pattern_pos
 ...
 *
 * @note  This function must be executed after fnc_uart_driver_install(), when the driver object is initialized.
 ...
 *        This function should be executed when actual transmission is completed (after fnc_uart_write_bytes()).
 *
 * @param uart_num  Uart number to configure the max port number is (UART_NUM_MAX -1).
 ...
 *
 * See description of fnc_uart_set_wakeup_threshold for the explanation of UART
 * wakeup feature.
 *
 * @param uart_num  UART number, the max port number is (UART_NUM_MAX -1).
...
/**
 * @brief UART bitrate detection configuration parameters for `fnc_uart_detect_bitrate_start` function to acquire a new uart handle
 */
typedef struct {
...
} fnc_uart_bitrate_detect_config_t;
