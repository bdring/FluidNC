...
bool ambiguousLimit() {
    if (Machine::Axes::posLimitMask & Machine::Axes::negLimitMask) {
        ...
        send({ Event::ALARM, ExecAlarm::HardLimit });
        return true;
    }
    return false;
}
...
void limits_soft_check(float* cartesian) {
    ...
    if (limit_error) {
        ...
        mc_reset();                                    // Issue system reset and ensure spindle and coolant are shutdown.
        send({ Event::ALARM, ExecAlarm::SoftLimit });  // Indicate soft limit critical event
...
void limitCheckTask(void* pvParameters) {
    while (true) {
        ...
        if (switch_state) {
            ...
            send({ Event::ALARM, ExecAlarm::HardLimit });  // Indicate hard limit critical event
        }
