...
#include "fnc_idf_uart_select_5_1.h"
...
esp_err_t fnc_uart_set_baudrate(uart_port_t uart_num, uint32_t baud_rate)
{
    ...
    ESP_RETURN_ON_ERROR(fnc_uart_get_sclk_freq(src_clk, &sclk_freq), UART_TAG, "Invalid src_clk");
...
esp_err_t fnc_uart_set_pin(uart_port_t uart_num, int tx_io_num, int rx_io_num, int rts_io_num, int cts_io_num)
{
    ...
    if (tx_io_num >= 0 && !fnc_uart_try_set_iomux_pin(uart_num, tx_io_num, SOC_UART_TX_PIN_IDX)) {
        gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[tx_io_num], PIN_FUNC_GPIO);
        gpio_set_level(tx_io_num, 1);
        esp_rom_gpio_connect_out_signal(tx_io_num, UART_PERIPH_SIGNAL(uart_num, SOC_UART_TX_PIN_IDX), 0, 0);
    }

    if (rx_io_num >= 0 && !fnc_uart_try_set_iomux_pin(uart_num, rx_io_num, SOC_UART_RX_PIN_IDX)) {
    ...
    if (rts_io_num >= 0 && !fnc_uart_try_set_iomux_pin(uart_num, rts_io_num, SOC_UART_RTS_PIN_IDX)) {
    ...
    if (cts_io_num >= 0  && !fnc_uart_try_set_iomux_pin(uart_num, cts_io_num, SOC_UART_CTS_PIN_IDX)) {
...
#endif
    uint32_t sclk_freq;
    ESP_RETURN_ON_ERROR(fnc_uart_get_sclk_freq(clk_src, &sclk_freq), UART_TAG, "Invalid src_clk");
...
static int fnc_uart_tx_all(uart_port_t uart_num, const char *src, size_t size, bool brk_en, int brk_len)
{
    ...
    if (p_uart_obj[uart_num]->tx_buf_size > 0) {
        ...
        while (size > 0) {
            ...
            size -= send_size;
            offset += send_size;
            fnc_uart_enable_tx_intr(uart_num, 1, UART_EMPTY_THRESH_DEFAULT);
        }
    } else {
        while (size) {
...
int fnc_uart_read_bytes(uart_port_t uart_num, void *buf, uint32_t length, TickType_t ticks_to_wait)
{
    ...
    while (length) {
        ...
        fnc_uart_pattern_queue_update(uart_num, len_tmp);
...
esp_err_t fnc_uart_flush_input(uart_port_t uart_num)
{
    ...
    while (true) {
        if (p_uart->rx_head_ptr) {
            ...
            fnc_uart_pattern_queue_update(uart_num, p_uart->rx_cur_remain);
            UART_EXIT_CRITICAL(&(uart_context[uart_num].spinlock));
            p_uart->rx_ptr = NULL;
