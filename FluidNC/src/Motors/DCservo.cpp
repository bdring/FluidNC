...
namespace MotorDrivers {
    ...
    void DCservo::set_operating_mode(uint8_t mode) {
    ...
    void DCservo::add_uint8(uint8_t n) { _tx_message[_msg_index++] = n & 0xff; }
    void DCservo::add_uint16(uint16_t n) {
        add_uint8(n);
        add_uint8(n >> 8);
    }
    void DCservo::add_uint32(uint32_t n) {
        add_uint16(n);
        add_uint16(n >> 16);
    }

    void DCservo::start_message(uint8_t id, uint8_t instr) {
        _msg_index = 0;
        add_uint8(0xFF);   // HDR1
        add_uint8(0xFF);   // HDR2
        add_uint8(0xFD);   // HDR3
    ...
    void DCservo::dxl_goal_position(int32_t position) {
        start_write(DXL_GOAL_POSITION);
        add_uint32(position);
        finish_write();
    }

    uint32_t DCservo::dxl_read_position() {
        uint16_t data_len = 4;
    ...
    void DCservo::dxl_read(uint16_t address, uint16_t data_len) {
    ...
    void DCservo::start_write(uint16_t address) {
        start_message(_id, DXL_WRITE);
        add_uint16(address);
    }
    void DCservo::finish_write() {
        finish_message();
        show_status();
    }
    void DCservo::LED_on(bool on) {
    ...
    uint16_t DCservo::dxl_update_crc(uint16_t crc_accum, uint8_t* data_blk_ptr, uint8_t data_blk_size) {
        uint16_t i, j;
