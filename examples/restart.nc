...
(check-mode dry run -- $C then $SD/Run=path,line or $LocalFS/Run=path,line --)
(reaches its target line. gc_state has been reconstructed by the dry run as of)
(that line, so #<_target_x/y/z>, #<_spindle_on/cw>, #<_rpm>, #<_flood>, and)
(#<_mist> reflect where the file's G-code would have left the tool and its)
(spindle/coolant state. Coolant and spindle are guaranteed off going into this,)
(since a dry run never actuates real hardware, so each action below only needs)
(an on branch, never an off branch.)
