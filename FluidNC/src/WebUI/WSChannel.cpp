...
namespace WebUI {
    ...
    //std::list<WSChannel*>             WSChannels::_wsChannelsList;
    ...
    WSChannel* WSChannels::getWSChannel(std::string session) {
        ...
        // Unless this is really needed for an unknown bug related to session IDs,
        // it would be better to not return potentially a random websocket from the list
        //if (_wsChannelsList.size() > 0)
        //    return _wsChannelsList.front();
        return nullptr;
    }
    ...
    void WSChannels::removeChannel(uint32_t num) {
        try {
            ...
            //_wsChannelsList.remove(wsChannel);
        ...
        switch (type) {
            ...
            case WS_EVT_CONNECT: {
                ...
                if (!wsChannel) {
                    ...
                    //_wsChannelsList.push_front(wsChannel);
                    log_debug_to(Uart0,
