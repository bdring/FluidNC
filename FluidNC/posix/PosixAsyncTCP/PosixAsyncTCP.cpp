...
size_t AsyncClient::add(const char* data, size_t size, uint8_t apiflags)
{
...

  {
    ...
    _send_buffer.push(std::string(data, will_send));
...
void AsyncClient::_handle_writable()
{
  ...
  if (sent >= 0 && _sent_cb) {
    if (sent > 0) {
      _sent_cb(_sent_cb_arg, this, sent, (millis() - _pcb_sent_at));
    }
  }
...
AsyncServer::AsyncServer(IPAddress addr, uint16_t port)
  : _port(port), _addr(addr), _noDelay(false), _listen_socket(-1)
{
}

AsyncServer::AsyncServer(uint16_t port)
  : _port(port), _addr(IPADDR_ANY), _noDelay(false), _listen_socket(-1)
{
}
