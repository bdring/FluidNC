...
struct fs::_Dir : _Dir_base {
    _Dir(const fs::path& p, bool skip_permission_denied, bool nofollow, [[maybe_unused]] bool filename_only, error_code& ec) :
    ...
    _Dir open_subdir(bool skip_permission_denied, bool nofollow, error_code& ec) const noexcept {
    ...
    bool do_unlink([[maybe_unused]] bool is_directory, error_code& ec) const noexcept {
#if _GLIBCXX_HAVE_UNLINKAT
        const auto atp = current();
        if (::unlinkat(atp.dir(), atp.path_at_dir(), is_directory ? AT_REMOVEDIR : 0) == -1) {
    ...
    bool unlink(error_code& ec) const noexcept { return do_unlink(/* is_directory*/ false, ec); }

    // Remove the directory that this->entry refers to.
    bool rmdir(error_code& ec) const noexcept { return do_unlink(/* is_directory*/ true, ec); }
